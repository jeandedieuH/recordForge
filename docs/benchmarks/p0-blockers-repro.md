...
| **P0.3** | System audio fails without Stereo Mix | Native audio | Manual test scenario in `docs/test-plans/capture-recovery.md` §5.2 | Resolved in code; manual validation pending |
| **P0.4** | Mic + system audio mixed in single track | Native audio | Manual test scenario in `docs/test-plans/capture-recovery.md` §5.2 | Resolved in code; manual validation pending |
...
- **Root Cause**: The old FFmpeg `-f dshow` path required an active virtual audio router or hardware "Stereo Mix" device.
- **Resolution**: Native WASAPI loopback now captures active Windows render endpoints directly, without DirectShow audio devices.
- **Impact**: Resolved in code; manual validation on a Windows 11 machine is still required.

### P0.4 — Mixed Audio Tracks

- **Root Cause**: The old FFmpeg filter graph combined microphone and system audio into one MP4 stream.
- **Resolution**: Native WASAPI writes `mic_NNN.wav` and `sys_NNN.wav`, then muxes them as separate AAC streams in each screen fragment.
- **Impact**: Resolved in code; manual validation on a Windows 11 machine is still required.
