...
On Windows, webcam capture retains DirectShow sample timestamps. The first encoded frame reports its original input PTS while the saved video is normalized to zero. Rust correlates the DirectShow graph clock with the capture process’s monotonic timeline using startup packet observations, then uses that estimated origin for the webcam segment offset without changing screen, microphone, or system-audio timing. `captureDiagnostics[].camera.cameraFirstFrameOffsetMs` records the optional camera-file origin relative to process spawn. Missing or unusable timing telemetry keeps the duration-derived fallback and records a stable `cameraTimingRejection` reason code (e.g. `sample-clock-skew`, `insufficient-samples`, `timestamps-unsupported`); other platforms and non-camera captures report `unsupported-backend` or omit the field. The clock correlation still includes residual pipe scheduling latency. Existing finalized webcam assets are not automatically realigned.
