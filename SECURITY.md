...
1. **OS Credential Manager Exclusivity:** S3 access keys, Google OAuth tokens, and sensitive secrets are never stored in SQLite, flat files, or logs. They are strictly committed to the **Windows Credential Manager**.
...
When reporting an updater problem, include the RecordForge version, Windows version, and the release tag or installer URL if known. Do not include updater private keys, tokens, local media paths, or recording content in an issue or security report.
